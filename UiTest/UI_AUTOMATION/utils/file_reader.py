...
class ExcelReader:
    ...
    | A1 | B1 | C1 |
    | A2 | B2 | C2 |
    如果 print(ExcelReader(excel, title_line=True).data1)，输出结果：
    [{A: A1, B: B1, C:C1}, {A:A2, B:B2, C:C2}]
    如果 print(ExcelReader(excel, title_line=False).data1)，输出结果：
...
if __name__ == '__main__':
    y = 'F:\\python\\soa-ui-master\\UI_AUTOMATION\\config\\config.yml'
    reader = YamlReader(y)
    print(reader.data)

    e = r'F:\\python\\soa-ui-master\\UI_AUTOMATION\\data1\\more_langrauge.xlsx'
    excel = ExcelReader(e)
    print(excel.data)
