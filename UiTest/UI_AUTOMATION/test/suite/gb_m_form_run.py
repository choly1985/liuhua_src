...
from UI_AUTOMATION.test.testcase.gb_m_form_case import FormTestCase
...
with open(report, "wb") as outfile:
    runner = HTMLTestRunner(stream=outfile, title=u"GB-M-PAY_UI表单校验测试报告", description=u"用例执行情况：", verbosity=2, retry=0, save_last_try=False)
    runner.run(suite)
